...
GROUP BY ORDERS.PARTNUM
HAVING PROSECEN_IZNOS > (SELECT  SUM(ORDERS.QUANTITY * PART.PRICE) FROM ORDERS NATURAL JOIN PART AS P WHERE P.PARTNUM = O.PARTNUM )

--5
--5.1 
CREATE VIEW UPLATA_VIEW AS SELECT * FROM UPLATA

--5.2
CREATE VIEW UPLATA_SMETKA_4 AS SELECT * FROM UPLATA WHERE SB# = 4;

--5.3
CREATE VIEW FIRMI_PODATOCI(FIRMA, POSTENSKA_ADRESA) AS SELECT ime, adresa + " " + grad + " " + drzava FROM FIRMA;

--5.4
CREATE VIEW UPLATI_DRZAVA AS SELECT FIRMA.drzava, COUNT(*), SUM(UPLATA.suma) FROM UPLATA NATURAL JOIN FIRMA GROUP BY drzava;

--5.5
CREATE VIEW ZADOCNETI_KAMATA AS SELECT ime, SB#, suma * (kamatna_stapka) FROM UPLATA WHERE zadocento = TRUE;
